    ...
    viewport={{once:true}}
      className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        {state != 'Login' && <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
            ...
            className='outline-none text-sm' placeholder='Full Name' required />
        </div>}
        ...
        <button className=' cursor-pointer bg-blue-600 w-full text-white py-2 rounded-full'>
        {state === 'Login' ? 'Login' : 'Create Account'}</button>

        { state === 'Login' ? <p className='mt-5 text-center'>Don't Have An Account?
        <span className='cursor-pointer text-blue-600' onClick={()=>setState('Sign Up')}>Sign Up </span></p>
        :
        <p className='mt-5 text-center'>Already Have An Account?
        <span onClick={()=>setState('Login')} className='cursor-pointer text-blue-600'>Login </span></p>}

        <img className='absolute top-5 cursor-pointer right-5' 
